...
export const useArcSlicer = () => {
  ...
  const [isAutoCranking, setIsAutoCranking] = useState(false);
  ...
  useEffect(() => {
    ...
    if (isAutoCranking && isParentInitialized) {
      logInfo('Auto-crank activated. Monitoring cadence.');
      
      // For testing purposes, this runs every 15 seconds. 
      // In production, you would map this to vaultData.cadenceSeconds
      intervalId = setInterval(() => {
        void turnCrank();
      }, 15000);
    } else if (!isAutoCranking && isParentInitialized) {
      logInfo('Auto-crank paused.');
    }

    return () => {
      if (intervalId) clearInterval(intervalId);
    };
  }, [isAutoCranking, isParentInitialized]); // Dependencies trigger the start/stop

  return {
    balances, vaultData, slices, isParentInitialized, isLoading, liveSolPrice, logs,
    initializeVault, turnCrank, depositFunds, buySlice, 
    isAutoCranking, setIsAutoCranking,
    refreshState: () => { 
      fetchProtocolState(false); 
      fetchBalances(); 
    } 
              ...
              {activeMode === 'whale' && (
                <>
                  ...
                  <div className="action-grid">
                    ...
                    <button 
                      className={`launch-btn setup-btn text-white disabled:opacity-50 ${isAutoCranking ? 'bg-red-600 hover:bg-red-500' : 'bg-purple-600 hover:bg-purple-500'}`} 
                      onClick={() => setIsAutoCranking(!isAutoCranking)} 
                      disabled={!isParentInitialized}
                    >
                      {isAutoCranking ? 'Stop Auto-Crank' : 'Start Auto-Crank'} <ArrowUpRight size={17} />
                    </button>
